...
function GoalList({ title, goals }: { title: string; goals: GoalItem[] }) {
  return (
    <div className="flex flex-col gap-3 sm:gap-5">
      <h3 className="inline-flex items-center gap-2 text-xl font-black uppercase tracking-[0.2em] text-[#FF8B00] sm:gap-3 sm:text-2xl lg:text-3xl lg:tracking-[0.25em] font-freeman glow-text-strong">
        {title}
      </h3>
      <div className="space-y-2 sm:space-y-3">
        {goals.map((goal) => (
          <div
            key={goal.label}
            className="group flex items-center gap-3 rounded-2xl border-3 border-[#ff9302] bg-black/60 backdrop-blur-md p-3 text-white glow-border glow-hover transition-all duration-200 hover:translate-y-2 sm:gap-4 sm:rounded-3xl sm:border-4 sm:p-4"
          >
...
export default function RoadmapSection() {
  return (
    <section id="roadmap" className="relative galaxy-bg py-10 text-white sm:py-16 lg:py-20">
